...
#cvlc  http://192.168.1.4:8082/stream.wmv
  ...
  2)  
    ...
    sleep 2
    cvlc -vvv ~/Desktop/non.mp3 --sout '#transcode{vcodec=h264,vb=800,scale=1,acodec=mpga,ab=128,channels=2,samplerate=44100}:http{mux=ts,dst=:8080/}'
    #Client from the command line use:
  ...
  3) 
    ...
    sleep 2 
    cvlc -vvv ~/Desktop/non.mp3 --sout '#rtp{dst=,sdp=rtsp://192.168.1.4:8080/test.sdp}'
    #Client from the command line use:replace with server ip
